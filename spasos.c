...
printf("\n\n -p   N Define a penalty factor (0 no penalty, 1 lowest, 5 highest).   Default 1 ");
...
pt_sp = sps_list ;
pt_conf = datamatrix ;
output_file = fopen ("lista.txt","w");
fprintf(output_file,"\nEspecie,individiuo,cat_alineada,cs");
...
for (  i = 0 ;   i < nuspecs ; i ++ )
 ...
 for (j = 0 ;  j < pt_sp[i].num_confs  ; j ++)
  {
  cual = pt_sp[ i ].conflist [ j ] ;
   fprintf(output_file,"\n%s,%i,%i,%f",pt_sp[i].sps_name, cual, pt_conf[cual].which_aln_class, pt_conf[cual].age);}}
fclose (output_file);
}
