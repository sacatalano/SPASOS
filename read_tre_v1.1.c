...
extern int trans_freq [MAXNODES][2];
extern double trans_relf [MAXNODES][2];
extern char treeFilename[300] ;
extern char fintreFilename [300] ;
extern char prefix [50];
char ** branch_labels, ** color_labels ;
char ** cats_labels ;
char ** sps_names ;
int sps , wrong = 0 ;
extern int nuspecs ;
...
extern int do_deca;
extern double support [MAXNODES][2];
extern int transformations [MAXNODES] ;
extern int easy_trans [MAXNODES ] [ 2] ;
int goto_printree_support (int ) ;
...
int   * numernod, * treenod, nunod,anod, c, especs,i ;
char   bufer [5000] ;
...
goto_printree_support (int place)
{

char nombre [300] ;
fprintf(caca_file,"\n tread ");
  printree_support (nt,place) ;
fprintf(caca_file," ; \n");
fflush(caca_file);
...
printree_support ( int nodo, int dde )
{
    int a, i  ;
    i = (nt * 2 )- 1 ;
    fprintf (caca_file,"(" ) ;
    for ( a = 0 ; a < (i) ; ++ a )
     if ( ances[a] == nodo )
      if ( a < nt ){
        fprintf(caca_file,"%i=%sNd%i|%s",a,color_labels[a],a,branch_labels[a]);
        if (easy_trans[a][dde] !=  0){
         if (do_resample && do_deca)
           fprintf(caca_file,"\\%.0f/%.1f",trans_relf [a][dde],support [a][dde]) ;//fprintf(caca_file,"\\%.0f/%.1f",trans_relf [a],support [a]) ;
         else
          if (do_deca)
           fprintf(caca_file,"\\-/%.1f ",support [a][dde]) ;
          else
           if (do_resample)
             fprintf(caca_file,"\\%.0f/-",trans_relf [a][dde] );}//fprintf(caca_file,"") ;
         fprintf(caca_file," ") ;
        }
      else
       printree_support ( a,dde );
   if (nodo == nt)
      fprintf (caca_file,")=");
   else{
    if (easy_trans[nodo][dde] != 0 ){
       fprintf (caca_file,")=%sNd%i|%s",color_labels[nodo],nod_p2t[nodo],branch_labels[nodo]);//fprintf (caca_file,")=%s%s\\Nd%i",color_labels[nodo],branch_labels[nodo],nod_p2t[nodo]);
       if (do_resample && do_deca) {
         fprintf (caca_file,"\\%.0f|%.1f",trans_relf [nodo][dde],support[nodo][dde]);} //fprintf (caca_file,"\\%.0f/%.1f",trans_relf[nodo],support[nodo]);}
       else
        if (do_deca)
          fprintf (caca_file,"\\-/%.1f",support[nodo][dde]);
        else
         if (do_resample)
          fprintf (caca_file,"\\%.0f/-",trans_relf[nodo][dde]);
       }
        else
           fprintf (caca_file,")=%sNd%i|%s",color_labels[nodo],nod_p2t[nodo],branch_labels[nodo]);//fprintf (caca_file,")=%s%s\\Nd%i",color_labels[nodo],branch_labels[nodo],nod_p2t[nodo]);
